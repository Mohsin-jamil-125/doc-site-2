...
class Loeschencompany extends Model  {
  ...
  public function firmLoeschen($params = false){
    if($params){
        ...
        if($insertReq){
          if($user = $this->db->get("allusers", array('select'=>'userfstname, userlstname, usermail, userphone', 'where'=>array('crt'=>$thisUser),'limit'=>1,'return_type'=>'single'))){	
            $updFirm = $this->db->update("firmen", array('isdel'=>1), array("usid"=>$thisUser));
            $updAnzeg = $this->db->update("anzeigen", array('ison'=>1), array("usid"=>$thisUser));							
            ...
            if(Main::websettings('mailFirmaDelete') == 1){ 
              $Firma = $this->db->get("firmen", array('select'=>'frmname', 'where'=>array('usid'=>$thisUser),'limit'=>1, 'return_type'=>'single'));
              if($user['userphone']){$userphone = str_replace('_normal', '', $user['userphone']); }else{ $userphone = ""; } 
              $emailDo = Main::emailCreate('email', array(     
                'customsubject'=>'Doc-Site.: Ein Unternehmen hat sein Konto gelöscht',
                'username'=>$Firma['frmname'], 
                'l1'=>"Gesellschaftsführer.: ".$user['userfstname'] .' '.$user['userlstname']."<br/><br/>Grund.: ".$frma["motiv"]."<br/>Grunddetails.: ".$frma["descrmotiv"],
