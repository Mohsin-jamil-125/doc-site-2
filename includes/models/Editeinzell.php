...
class Editeinzell extends Model  {
  ...
  public function editAnzeigge($params = false){
    if($params){
          ...
          if(!$aktivate = $this->doSetAktive($thisUser, $anzID, $RechnungID)){
            ...
            if($aktivate['stat'] == 4){
              ...
              if($getAnzeigeDates = $this->db->get("paystats", array("select"=>"datactivat, gultigbis", "where"=>array("anzid"=>$anzID, "billnr"=>$RechnungID), "limit"=>1, "return_type"=>"single"))){
                if($getAnzeigeDates['datactivat'] && $getAnzeigeDates['gultigbis']){
                  ...
                  $updAnzeigeStats = $this->db->update("paystats", array("ison"=>3, "datactivat"=>date("Y-m-d H:i:s"), "gultigbis"=>$gultig_bis, "sichtbarbis"=>$sichtbar_bis), array('billnr'=>$RechnungID, 'usid'=>$thisUser, 'anzid'=>$anzID));
                ...
                if($RechnungID != $OLD_Rechnung){ // a schimbat planul
                  ...
                  $updAnzeigeStats = $this->db->update("paystats", array("ison"=>3, "datactivat"=>date("Y-m-d H:i:s"), "gultigbis"=>$gultig_bis, "sichtbarbis"=>$sichtbar_bis), array('billnr'=>$OLD_Rechnung, 'usid'=>$thisUser, 'anzid'=>$anzID));
                }
       ...
       /* if($linksoccanzeigg = is_url_extended($linkanzeigg)){
          $parse = parse_url($companyURL);
  ...
  public function anzEdLogo($files, $posted){
    if(!isset($files['anzeglogoFirma']['name']) || empty($files['anzeglogoFirma']['name']) || !$files['anzeglogoFirma']['size'] > 0){
    ...
    $thisIdNr = Main::decrypt(filter_var($_POST['anzregtoken'], FILTER_SANITIZE_STRING));
    ...
    $ismoved = $resize->doimages("anzeglogoFirma", ROOT."/".$targetDir, 350, 350);
    if($ismoved['type'] == 2){                        
