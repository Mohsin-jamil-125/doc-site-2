...
class ProfilanlegenController extends Controller {
    ...
    public function __construct($db, $config){
       ...
       $this->model = $this->model('Profilanlegen', $this->config, $this->db);
    }
...
//        $this->model->checkIsLogged();
    ...
    public function replacemail($params = ''){
        if(Main::bot()) return $this->content("_404");
        $data = json_decode(file_get_contents("php://input"));
        if($p = $this->model->doReplaceMail($data->doit)){
            http_response_code(200);
            echo json_encode($p);
        }else{
            http_response_code(200);
